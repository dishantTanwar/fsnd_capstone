...
# --------------------------------------------
    ...
    def root_index():
        html_index = '<h1>Capstone APP running</h1>'
        return html_index
    ...
    def add_actor(payload):
        ...
        # only verifying name because others have a default value of 'Not
        # disclosed'
        if not name:
            abort(400, {'message': 'actor name missing from request'})

        actor = Actor(name=name, age=age, gender=gender,
                      msg_to_world=msg_to_world)
        try:
            actor.insert()
            return jsonify({
                "success": True,
    ...
    def update_actor(payload, id):
        ...
        actor = Movie.query.get_or_404(
            id, {'message': 'Resource id not found.'})
